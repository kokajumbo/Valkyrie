...
Процедура _ОК(Команда)
	...
	Если ИмяРегистра = "НастройкиУчетаНДС"
		И Не КорректныйПериодНачалаВеденияРаздельногоУчетаНДС() Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ИмяРегистра = "НастройкиУчетаНДС" Тогда
		...
		Если (Не ОрганизацияВелаРаздельныйУчет И ЭтотОбъект.НастройкиУчетаНДС.РаздельныйУчетНДСНаСчете19) 
