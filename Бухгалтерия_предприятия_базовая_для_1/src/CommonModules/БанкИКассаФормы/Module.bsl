...
Процедура ПодготовитьРасчетыПриОплате(Форма, НазваниеТабличнойЧасти) Экспорт
	
	ИзменитьРасчетыПриОплате(Форма, Ложь);
	ЗаполнитьСписокВыбораПорядкаОтраженияДоходаВТЧ(Форма, НазваниеТабличнойЧасти);
	
КонецПроцедуры

Процедура УстановитьРасчетыПриОплате(Форма, НазваниеТабличнойЧасти) Экспорт
...
// Устанавливает значение реквизитов РасчетыПриОплате и БезЗакрывающихДокументов в зависимости от настроенных опций.
//
// Параметры:
//   Форма                              - УправляемаяФорма
//   УстановитьБезЗакрывающихДокументов - Булево
//
Процедура ИзменитьРасчетыПриОплате(Форма, УстановитьБезЗакрывающихДокументов) Экспорт
	...
	УчетБезДокументовВозможен = УчетДенежныхСредствБП.УчетБезЗакрывающихДокументовВозможен(Объект.Организация, Объект.Дата);
...
Процедура ЗаполнитьСписокВыбораПорядкаОтраженияДоходаВТЧ(Форма, НазваниеТабличнойЧасти) Экспорт
	
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	
	Если НЕ Объект.БезЗакрывающихДокументов 
		ИЛИ (НЕ УчетДенежныхСредствКлиентСервер.ЕстьРасчетыСПокупателями(Объект.ВидОперации)
			И Не УчетДенежныхСредствКлиентСервер.ЕстьРозничнаяВыручка(Объект.ВидОперации)) Тогда
		Форма.ПолеОтражениеДоходаВидимость = Ложь;
		Возврат;
	КонецЕсли;
	
	ЭлементПолеОтражениеДохода = Элементы.Найти(НазваниеТабличнойЧасти + "ПолеОтражениеДохода");
	Если ЭлементПолеОтражениеДохода <> Неопределено Тогда
		НалоговыйУчетУСН.ЗаполнитьСписокВыбораПорядкаОтраженияАвансовДляТЧ(
			ЭлементПолеОтражениеДохода,
			Форма.ПорядокОтраженияДоходаСоответствиеЗначений,
			Объект);
		Форма.ПолеОтражениеДоходаВидимость = Форма.ПорядокОтраженияДоходаСоответствиеЗначений.Количество() > 1;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеБезЗакрывающихДокументов(Форма, НачалоПутиКДанным) Экспорт
	...
	ДобавитьПолеВУсловноеОформление("РасшифровкаПлатежаСчетУчетаРасчетовПоАвансамПриУСН", ЭлементУО, Элементы);
	ДобавитьПолеВУсловноеОформление("РасшифровкаПлатежаПолеОтражениеАванса", ЭлементУО, Элементы);
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаПолеОтражениеДохода");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ЭлементУО.Отбор,
