...
Процедура ЗарегистрироватьВыплатуВУчетеНДФЛ(Ведомость, Отказ) Экспорт
	...
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Организация = Ведомость.Организация;
	...
	КоллекцияДвижений = ЗарплатаКадры.НаборыЗаписейРегистратора(Метаданные.Документы.ОперацияНалоговогоУчетаПоНДФЛ);
	УчетНДФЛ.СформироватьУдержанныйНалогПоВременнойТаблице(КоллекцияДвижений, Отказ, Организация, ДатаВыплаты, МенеджерВременныхТаблиц,,Ложь);
	
	УчетНДФЛ.СформироватьНДФЛКПеречислению(Ведомость.Движения, Отказ, КоллекцияДвижений);
	ОтражениеЗарплатыВУчете.ДополнитьНДФЛКПеречислениюСведениямиОРаспределенииПоСтатьям(Ведомость.Движения, ДанныеДляДополнения);
