...
// 
// Параметры:
//	ПроверяемаяФорма     - УправляемаяФорма - форма, заполнение реквизитов которой проверяется.
...
Функция ОрганизацияЗаполнена(ПроверяемыйОбъект, Знач ТекстПредупреждения = Неопределено, Знач Заголовок = Неопределено) Экспорт
	...
	ОрганизацияЗаполнена = ЗначениеЗаполнено(ПроверяемыйОбъект["Организация"]); // не локализуется
