...
Процедура ЗаполнитьСоставСотрудниковДокумента(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока Истина Цикл
		...
		Для Каждого ТипДокумента Из ТипыДокументовФизическихЛиц Цикл
			...
			Запрос = Новый Запрос;
			Запрос.Текст = ЗапросДокументовКОбработке(Документ.ПолноеИмя(), ОписаниеДокумента, КоличествоКОбработке);
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЧастиЗапроса, ЧастиЗапросаПоТаблицамДокумента(Документ.ПолноеИмя(), ОписаниеДокумента));
...
Функция ЗапросДокументовКОбработке(ИмяДокумента, ОписаниеДокумента, КоличествоКОбработке)
	...
	ТекстЗапросаДокументов = СтрЗаменить(ТекстЗапросаДокументов, "#ДокументФизическихЛиц", ИмяДокумента);
	УсловиеТаблицы = "
		|	НЕ 1 В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		|				1
