...
Функция ИнтервалМеждуЗапросамиСписковВыдержан(ПараметрыОбмена, Операция) Экспорт
	...
	Если ЭтоЗапросСписка(Операция) Тогда
		...
		Если ПараметрыОбмена.ОжидатьНаСервере Тогда
			Ожидать(ВремяОжиданияСледующегоЗапроса, ПараметрыОбмена);
		КонецЕсли;
...
Функция ПостроитьОчередьСообщений(ПараметрыОбмена, СоответствиеЗаписиСкладскогоЖурнала = Неопределено, СообщенияКПередаче = Неопределено) Экспорт
	...
	Для Каждого СтрокаТЧ Из ЗависимостиСообщений Цикл
		Если ОчередьСообщений.Найти(СтрокаТЧ.Сообщение1, "Сообщение") = Неопределено
			Или ОчередьСообщений.Найти(СтрокаТЧ.Сообщение2, "Сообщение") = Неопределено Тогда
			ЗависимостиКУдалению.Вставить(СтрокаТЧ, Истина);
		КонецЕсли;
	КонецЦикла;
	Для Каждого КлючИЗначение Из ЗависимостиКУдалению Цикл
		ЗависимостиСообщений.Удалить(КлючИЗначение.Ключ);
	КонецЦикла;
...
//
Процедура ЗаполнитьНомераПроизводителей(Производители, Знач Производитель = Неопределено) Экспорт
...
Процедура ПриСозданииНаСервереФормыСпискаДокументовВЕТИС(Форма, ИменаСписков, ИменаСписковКОформлению) Экспорт
	...
	Для Каждого ИмяСписка Из Новый Структура(ИменаВсехСписков) Цикл
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
			...
			ИнтеграцияИС.НезаполненныеЗначенияОпределяемогоТипа("ПроизводственныйОбъектВЕТИС"));
...
Функция ЗапросДляПроверкиКорректностиОформленияXML(ДокументСсылка) Экспорт
	...
	ПараметрыОбмена = ИнтеграцияВЕТИС.ПараметрыОбмена(ХозяйствующийСубъект, Предприятие);
	...
	Если РеквизитыСообщения.Следующий() Тогда
		...
		СообщенияЗаписейСкладскогоЖурналяXML = ЗаявкиВЕТИС.ПодготовитьЗапросИзмененныхЗаписейСкладскогоЖурналаXML(
			ХозяйствующийСубъект,
			Предприятие,
			ПараметрыОбмена.РазмерПорции,
			ПараметрыОбмена,
		...
		Для Каждого СообщениеXML Из СообщенияЗаписейСкладскогоЖурналяXML Цикл
			Сообщение = Новый Структура;
			Сообщение.Вставить("СообщениеXML",       СообщениеXML);
...
Процедура ЗафиксироватьОшибкуСервиса(ЭлементОчереди, ПараметрыОбмена) Экспорт
	...
	Если ПараметрыОбмена.ДокументыДляВосстановления[ТипДокумента] = Неопределено Тогда
		ПараметрыОбмена.ДокументыДляВосстановления[ТипДокумента] =
			ИнтеграцияВЕТИС.ИнициализироватьДокументыДляВосстановления(ТипДокумента);
	КонецЕсли;
...
// Возвращаяет структуру с именами полей и их типами, по которым будет производится подбор
...
// Возвращаяет таблицу с документами, требующими восстановления в следствии ошибок сервиса.
// 
// Параметры:
// Возвращаемое значение:
...
Функция ДокументыДляВосстановления(ТипДокумента, ОрганизацииПредприятияВЕТИС = Неопределено) Экспорт
	...
	ПоляОтбора = ИнтеграцияВЕТИС.ПоляПодбораДокументовПриВосстановлении(ТипДокумента);
...
Функция СостояниеОбмена(ОрганизацииВЕТИС, Документ = Неопределено) Экспорт
	...
	Интервал = Константы.ИнтервалЗапросаИзмененныхДанныхВЕТИС.Получить();
