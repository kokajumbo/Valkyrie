...
Процедура ЗаполнениеДокументовВЕТИС(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
...
Процедура ЗаполнитьСырьеПродукцию(ДокументОбъект, ДанныеЗаполнения)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПродукцияСерии.Номенклатура          КАК Номенклатура,
	|	ПродукцияСерии.Характеристика        КАК Характеристика,
	|	ПродукцияСерии.Серия                 КАК Серия,
	|	ПродукцияСерии.Количество            КАК Количество
	|ПОМЕСТИТЬ ПродукцияКОформлению
	|ИЗ
	|	Продукция КАК ПродукцияСерии
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МатериалыСерии.Номенклатура          КАК Номенклатура,
	|	МатериалыСерии.Характеристика        КАК Характеристика,
	|	МатериалыСерии.Серия                 КАК Серия,
	|	МатериалыСерии.Количество            КАК Количество
	|ПОМЕСТИТЬ МатериалыКОформлению
	|ИЗ
	|	Материалы КАК МатериалыСерии
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ТабличнаяЧасть.Характеристика КАК Характеристика,
	|	ТабличнаяЧасть.Серия КАК Серия
	|ПОМЕСТИТЬ СопоставленнаяПродукция
	|ИЗ
	|	ПродукцияКОформлению КАК ТабличнаяЧасть
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК СоответствиеНоменклатурыВЕТИС
	|		ПО СоответствиеНоменклатурыВЕТИС.Номенклатура = ТабличнаяЧасть.Номенклатура
	|			И СоответствиеНоменклатурыВЕТИС.Характеристика = ТабличнаяЧасть.Характеристика
	|			И (СоответствиеНоменклатурыВЕТИС.Серия = ТабличнаяЧасть.Серия)
	...
	|	ТабличнаяЧасть.Характеристика КАК Характеристика,
	|	ТабличнаяЧасть.Серия КАК Серия
	|ПОМЕСТИТЬ СопоставленныеМатериалы
	|ИЗ
	|	МатериалыКОформлению КАК ТабличнаяЧасть
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК СоответствиеНоменклатурыВЕТИС
	|		ПО СоответствиеНоменклатурыВЕТИС.Номенклатура = ТабличнаяЧасть.Номенклатура
	|			И СоответствиеНоменклатурыВЕТИС.Характеристика = ТабличнаяЧасть.Характеристика
	|			И (СоответствиеНоменклатурыВЕТИС.Серия = ТабличнаяЧасть.Серия)
	...
	|ИЗ
	|	ПродукцияКОформлению КАК ТоварыКОформлению
	|		ЛЕВОЕ СОЕДИНЕНИЕ СопоставленнаяПродукция КАК СопоставленыеПозиции
	|		ПО СопоставленыеПозиции.Номенклатура = ТоварыКОформлению.Номенклатура
	|			И СопоставленыеПозиции.Характеристика = ТоварыКОформлению.Характеристика
	|			И СопоставленыеПозиции.Серия = ТоварыКОформлению.Серия
	|СГРУППИРОВАТЬ ПО
	...
	|	МатериалыКОформлению.Серия,
	|	СопоставленныеМатериалы.Продукция,
	|	СУММА(МатериалыКОформлению.Количество),
	|	СУММА(МатериалыКОформлению.Количество),
	|	Ложь
	|ИЗ
	|	МатериалыКОформлению КАК МатериалыКОформлению
	|		ЛЕВОЕ СОЕДИНЕНИЕ СопоставленныеМатериалы КАК СопоставленныеМатериалы
	|		ПО СопоставленныеМатериалы.Номенклатура = МатериалыКОформлению.Номенклатура
	|			И СопоставленныеМатериалы.Характеристика = МатериалыКОформлению.Характеристика
	|			И СопоставленныеМатериалы.Серия = МатериалыКОформлению.Серия
	|СГРУППИРОВАТЬ ПО
	|	МатериалыКОформлению.Номенклатура,
	|	МатериалыКОформлению.Характеристика,
	|	МатериалыКОформлению.Серия,
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Если ВыборкаНоменклатура.ЭтоПродукция Тогда
			НоваяСтрока = ДокументОбъект.Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаНоменклатура);
			УстановитьИдентификаторСтроки(НоваяСтрока, Истина);
			ИнтеграцияВЕТИС.ПроверитьОчиститьЕдиницуИзмеренияВЕТИС(НоваяСтрока);
