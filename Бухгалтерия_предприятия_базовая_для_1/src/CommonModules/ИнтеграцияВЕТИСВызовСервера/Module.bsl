...
Функция ВыполнитьОбмен(ХозяйствующиеСубъекты = Неопределено, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если УникальныйИдентификатор <> Неопределено И Не РежимРаботыБезФоновыхЗаданий() Тогда
		...
		ПараметрыВыполнения.КлючФоновогоЗадания         = Строка(УникальныйИдентификатор);
...
Функция ПродолжитьВыполнениеОбмена(АдресВоВременномХранилище) Экспорт
	...
	Если ПараметрыОбмена.УникальныйИдентификатор <> Неопределено И Не РежимРаботыБезФоновыхЗаданий() Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Выполнение обмена с ВетИС'");
		ПараметрыВыполнения.КлючФоновогоЗадания         = Строка(ПараметрыОбмена.УникальныйИдентификатор);
...
Функция ВыполнитьОбменУниверсально(ПараметрыОбмена) Экспорт
	...
	Если ТребуетсяОжидание Тогда
		...
		Если ПараметрыОбмена.ОжидатьНаСервере Тогда
			ИнтеграцияВЕТИС.Ожидать(ВремяОжидания, ПараметрыОбмена);
...
Функция ВремяОжиданияИтерации(НомерИтерации)
	...
	Если НомерИтерации < 15 Тогда
		ВремяОжидания = 1;
		Для НомерИтерации = 2 По НомерИтерации Цикл
...
// Описание
// Формирует таблицу по ошибкам, возаращенным сервисом для документа.
// Параметры:
...
// * ОписаниеОшибки - String - описание ошибки.
// * КодОшибки - String - код возвращаемой сервисом ошибки.
Функция ПолучитьТаблицуОшибок(Документ) Экспорт
	
	ТаблицаОшибок = Неопределено;
	
	СообщениеСОшибкой = ИнтеграцияВЕТИСВызовСервера.ПоследнееСообщение(Документ, "СОшибкой");
	Если СообщениеСОшибкой <> Неопределено Тогда
		ТекстСообщенияXML = ИнтеграцияИС.ТекстСообщенияXMLИзПротокола(СообщениеСОшибкой.Сообщение);
		ТаблицаОшибок = ДанныеОшибкиТаблицей(ТекстСообщенияXML);
	КонецЕсли;
