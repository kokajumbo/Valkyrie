...
Функция СтруктураРеквизитыРасчетаСтатусаОформления()
	...
	Реквизиты.Вставить("Ответственный", Справочники.Пользователи.ПустаяСсылка());
	Реквизиты.Вставить("ТорговыйОбъект", Справочники.Склады.ПустаяСсылка());
...
Процедура РассчитатьСтатусОформленияДокументаЕГАИС(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.АктПостановкиНаБалансЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.АктСписанияЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ВозвратИзРегистра2ЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ПередачаВРегистр2ЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ТТНВходящаяЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ТТНИсходящаяЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ЧекЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ЧекЕГАИСВозврат") Тогда
		
		ИсточникОбъект = Источник;
		ИсточникСсылка = Источник.Ссылка;
		
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументСсылка.АктПостановкиНаБалансЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументСсылка.АктСписанияЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументСсылка.ВозвратИзРегистра2ЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументСсылка.ПередачаВРегистр2ЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументСсылка.ТТНИсходящаяЕГАИС")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументСсылка.ЧекЕГАИС")
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ИсточникОбъект, "ДокументОснование") Тогда
		ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсточникСсылка, "ДокументОснование");
	Иначе
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОприходованиеТоваров")
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.СписаниеТоваров") 
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") 
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") 
		...
		|ИЗ
		|	" + ДокументОснование.Метаданные().ПолноеИмя() + " КАК Таблица
		|ГДЕ
		...
		ЗаписьТребуется = ТребуетсяОформлениеДокументовЕГАИС(
			ДокументОснование,
			ИсточникОбъект.Метаданные());
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПередачаВРегистр2ЕГАИС") 
		...
		|	Таблица.Номер          КАК Номер,
		|	Таблица.Ответственный  КАК Ответственный,
		|	Таблица.ТорговыйОбъект КАК ТорговыйОбъект,
		|	Таблица.Организация    КАК Контрагент
		|ИЗ
		|	" + ДокументОснование.Метаданные().ПолноеИмя() + " КАК Таблица
		|ГДЕ
	...
	ИнтеграцияЕГАИС.РассчитатьСтатусОформления(
		ИсточникОбъект.Метаданные(),
		ДокументОснование, ЗначенияРеквизитов, ЗаписьТребуется);
...
Процедура ОбработатьОснованияЕГАИС(Параметры) Экспорт
	Запрос = Новый Запрос;
	
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	ДокументТТНВходящаяЕГАИС.Ссылка КАК Ссылка,
	|	ДокументПоступлениеТоваровУслуг.Ссылка КАК ПоступлениеТоваровУслуг
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг КАК ДокументПоступлениеТоваровУслуг
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ТТНВходящаяЕГАИС КАК ДокументТТНВходящаяЕГАИС
	|		ПО ДокументПоступлениеТоваровУслуг.УдалитьТТНВходящаяЕГАИС = ДокументТТНВходящаяЕГАИС.Ссылка";
	...
	Для каждого СтрокаРезультата Из РезультатЗапроса Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", СтрокаРезультата.ПоступлениеТоваровУслуг);
			
			Блокировка.Заблокировать();
			
			ДокументОбъектЕГАИС = СтрокаРезультата.Ссылка.ПолучитьОбъект();
			Если ДокументОбъектЕГАИС = Неопределено Тогда
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			
			ДокументОбъектБП = СтрокаРезультата.ПоступлениеТоваровУслуг.ПолучитьОбъект();
			Если ДокументОбъектБП = Неопределено Тогда
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ДокументОбъектЕГАИС.ДокументОснование) Тогда
				ДокументОбъектЕГАИС.ДокументОснование = СтрокаРезультата.ПоступлениеТоваровУслуг;
				ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ДокументОбъектЕГАИС);
			КонецЕсли;
			
			ДокументОбъектБП.УдалитьТТНВходящаяЕГАИС = Документы.ТТНВходящаяЕГАИС.ПустаяСсылка();
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ДокументОбъектБП);
			
			ИнтеграцияЕГАИСПереопределяемый.РассчитатьСтатусОформленияТТНВходящаяЕГАИС(ДокументОбъектЕГАИС, Ложь);
