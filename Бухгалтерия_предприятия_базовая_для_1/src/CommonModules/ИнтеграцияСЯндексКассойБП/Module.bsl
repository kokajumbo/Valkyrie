...
Процедура ЗаполнитьДанныеОснованияПлатежа(ОснованиеПлатежа, ДанныеОснованияПлатежа) Экспорт
	...
	Если Не Результаты[0].Пустой() Тогда
		...
		СведенияОбОрганизации = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(Выборка.Организация,
			Выборка.Дата, Выборка.БанковскийСчет);
		СведенияОКонтрагенте = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(Выборка.Контрагент, Выборка.Дата);
		
		НомерДокумента = ПечатьТорговыхДокументов.НомерСчетаНаОплату(Выборка.НомерГосударственногоКонтракта, Выборка.Номер);
		
		ДанныеОснованияПлатежа.Идентификатор = ИдентификаторПлатежа(Выборка.Номер, Выборка.СсылочныйИдентификатор);
		ДанныеОснованияПлатежа.Номер = Выборка.Номер;
		ДанныеОснованияПлатежа.Сумма = Выборка.СуммаДокумента;
		ДанныеОснованияПлатежа.Валюта = Выборка.ВалютаДокументаКод;
		ДанныеОснованияПлатежа.НазначениеПлатежа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Оплата по счету %1 от %2'"),
			НомерДокумента, Формат(Выборка.Дата, "ДЛФ=D"));
		...
		ДанныеОснованияПлатежа.Продавец.УчетнаяПолитика = КодСистемыНалогообложения(Выборка.Организация, Выборка.Дата);
		...
		ДанныеОснованияПлатежа.Продавец.Наименование = УчетДенежныхСредствБП.СформироватьТекстНаименованияПлательщикаПолучателя(,
			Выборка.Организация, Выборка.БанковскийСчет,, Выборка.Дата);
		...
		ДанныеОснованияПлатежа.Покупатель.Идентификатор = ИдентификаторПокупателя(Выборка.Контрагент);
	...
	Если НЕ Результаты[1].Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаНоменклатуры.Наименование = Выборка.Наименование;
			СтрокаНоменклатуры.НаименованиеПолное = Выборка.НаименованиеПолное;
			СтрокаНоменклатуры.Характеристика = Выборка.Характеристика;
			СтрокаНоменклатуры.Количество = Выборка.Количество;
			СтрокаНоменклатуры.Цена = Выборка.Цена;
			СтрокаНоменклатуры.СтавкаНДС = Выборка.СтавкаНДС;
			СтрокаНоменклатуры.СтавкаНДСКод = Выборка.СтавкаНДСКод;
			СтрокаНоменклатуры.Валюта = Выборка.Валюта;
			СтрокаНоменклатуры.Артикул = Выборка.Артикул;
			СтрокаНоменклатуры.ЕдиницаИзмерения = Выборка.ЕдиницаИзмерения;
			СтрокаНоменклатуры.ВидНоменклатуры = Выборка.ВидНоменклатуры;
			СтрокаНоменклатуры.Родитель = Выборка.Родитель;
			СтрокаНоменклатуры.Сумма = Выборка.Сумма;
