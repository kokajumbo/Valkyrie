...
Функция СхемаРасчета(КонтекстРасчета, ОписаниеСумм)
	...
	ДобавитьРасчет(СхемаРасчета, "УбыткиПрошлыхЛет", Ложь);
	...
	Для ИндексПериода = 0 По КоличествоСтрок - 1 Цикл
		...
		ДобавитьСлагаемое(СхемаРасчета, ИмяСуммаУбытка);
		...
		ИсточникДанных = ДобавитьИсточникДанных(СхемаРасчета, "УбыткиПрошлыхЛет");
		ИсточникДанных.ОтборДанных.ИндексПериода = ИндексПериода;
		ИсточникДанных.ОтборДанных.Показатель    = "Сумма";
		
		// номера лет не должны участвовать в расчете суммовых показателей; они имеют служебные номера строк
		...
		ИсточникДанных = ДобавитьИсточникДанных(СхемаРасчета, "УбыткиПрошлыхЛет");
		ИсточникДанных.ОтборДанных.ИндексПериода = ИндексПериода;
		ИсточникДанных.ОтборДанных.Показатель    = "НомерГода";
		
		// вернемся к расчету показателя "УбыткиПрошлыхЛет", равного сумме по годам
...
Функция СоставДекларации(КонтекстРасчета)
	...
	Если ЭтоКвартал Тогда
		...
		НомерКвартала = ОтчетныйМесяц / 3;
		Если НомерКвартала = 1 Или НомерКвартала = 4 Тогда
...
Процедура ПолучитьДанныеИнформационнойБазы(ПроцессорПолученияДанных)
	...
	ИначеЕсли Алгоритм = "УбыткиПрошлыхЛет" Тогда
...
Функция НовыйОтборДанных(ИмяАлгоритма)
	...
	ИначеЕсли ИмяАлгоритма = "УбыткиПрошлыхЛет" Тогда
		ОтборДанных.Вставить("ИндексПериода", 0);
		ОтборДанных.Вставить("Показатель",    "");//"Сумма", "НомерГода"
	КонецЕсли;
...
Функция НастройкиОтчетаРасшифровкаЗначения(ЗаписьПротокола, СхемаРасчета)
	...
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = "УбыткиПрошлыхЛет" Тогда
...
Процедура ПолучитьАмортизация(ПроцессорПолученияДанных)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , , , СчетКт В (&СчетаОсновныеСредства), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства), Организация В (&Организации)) КАК ОсновныеСредства
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	""НематериальныеАктивы"",
	|	ЕСТЬNULL(СУММА(НематериальныеАктивы.СуммаНУОборотДт), 0),
	|	ЕСТЬNULL(СУММА(НематериальныеАктивы.СуммаНУОборотКт), 0) - ЕСТЬNULL(СУММА(НематериальныеАктивы.СуммаНУОборотДт), 0)
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , , , СчетКт В (&СчетаНематериальныеАктивы), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НематериальныеАктивы), Организация В (&Организации)) КАК НематериальныеАктивы";
...
Процедура ПолучитьУбыткиПрошлыхЛет(ПроцессорПолученияДанных)
	...
	// Форма предусматривает ограниченное число строк убытка.
	//
	// Получим эти строки - сгруппируем задачи получения данных по ним.
	// Строки должны иметь индексы без пропусков, начиная с 0, и оба показателя ("Сумма", "НомерГода")
	//
	// Какая строка какому году соответствует, будет определено позднее - при выборке данных.
	// При этом данные за "непоместившиеся" годы будут включены в первую строку.
	СтрокиПериодовУбытка  = Новый Соответствие;
	Для Каждого Задача Из ПроцессорПолученияДанных.Задачи Цикл
		...
		ОписаниеСтроки = СтрокиПериодовУбытка[Задача.ОтборДанных.ИндексПериода];
		
		Если ОписаниеСтроки = Неопределено Тогда
			ОписаниеСтроки = Новый Структура;
			СтрокиПериодовУбытка.Вставить(Задача.ОтборДанных.ИндексПериода, ОписаниеСтроки);
		КонецЕсли;
		
		ОписаниеСтроки.Вставить(Задача.ОтборДанных.Показатель, Задача);
		
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(СтрокиПериодовУбытка) Тогда
		Возврат;
	КонецЕсли;
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ПерваяСтрока = СтрокиПериодовУбытка[0];
		...
		Для Каждого Шаблон Из ШаблоныОписаний Цикл
		
			ЗаписьПротокола = ДобавитьЗаписьПротоколаПолученияДанныхИнформационнойБазы(ПроцессорПолученияДанных, ПерваяСтрока[Шаблон.Ключ]);
			ЗаписьПротокола.Описание = СтрШаблон(
				Шаблон.Значение,
	...
	КоличествоСтрокФормы = СтрокиПериодовУбытка.Количество();
	КоличествоДополнительныхЛетПервойСтроки = 0;
	КоличествоЛетУбытка = Выборка.Количество();
	Если КоличествоЛетУбытка > КоличествоСтрокФормы Тогда
		КоличествоДополнительныхЛетПервойСтроки = КоличествоЛетУбытка - КоличествоСтрокФормы;
	КонецЕсли;
	...
	ИндексСтрокиУбытка         = 0;
	ПоследняяЗаполненнаяСтрока = -1;
	Пока Выборка.Следующий() Цикл
		
		ПоследняяЗаполненнаяСтрока = ИндексСтрокиУбытка;
		
		ОписаниеСтроки = СтрокиПериодовУбытка[ИндексСтрокиУбытка];
		ОписаниеСтроки.Сумма.СуммаНУ     = ОписаниеСтроки.Сумма.СуммаНУ + Выборка.СуммаНУ;
		ЗаписатьНомерГода(ОписаниеСтроки.НомерГода, Выборка.ПериодВозникновения, ПроцессорПолученияДанных.ОписаниеСумм);
		
		ЗаписьПротокола = ДобавитьЗаписьПротоколаПолученияДанныхИнформационнойБазы(ПроцессорПолученияДанных, ОписаниеСтроки.Сумма, Выборка);
		ЗаписьПротокола.Описание = СтрШаблон(
			ШаблонДетальногоОписания,
		...
		Если КоличествоДополнительныхЛетПервойСтроки > 0 Тогда
			КоличествоДополнительныхЛетПервойСтроки = КоличествоДополнительныхЛетПервойСтроки - 1;
		Иначе
			ИндексСтрокиУбытка = ИндексСтрокиУбытка + 1;
		КонецЕсли;
...
Процедура ЗаписатьНомерГода(Задача, Период, ОписаниеСумм)
	...
	Задача[ИменаСвойств[0]] = Год(Период);
	
КонецПроцедуры

Функция ПрочитатьНомерГода(Задача, ОписаниеСумм)
	...
	НомерГода = Задача[ИменаСвойств[0]];
...
Процедура ПередатьРассчитанныеЗначенияБРО(Контейнер, РассчитанныеЗначения, ОписаниеФормыОтчета, ИмяРедакции, СхемаРасчета)
	...
	ЗаполненныеСтраницы = Новый Соответствие; // Ключ - имя раздела БРО
	...
	Для Каждого Показатель Из ОписаниеФормыОтчета Цикл
		...
		Если Показатель.ОсобенностьПредставления = "НомерГода" Тогда
			...
			Значение = РассчитанноеЗначение(РассчитанныеЗначения, Показатель.Имя);
			Если Значение <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Значение)
			...
			ЗаполненныеСтраницы.Вставить(ИмяРазделаБРО, Истина);
	...
	Для Каждого ОписаниеРаздела Из СоставДекларации Цикл
		...
		РазделБРО = Контейнер[ИмяРаздела];
		ВключитьСтраницу(РазделБРО[0], ОписаниеРаздела, ИмяРедакции);
	...
	Для Каждого Показатель Из ОписаниеФормыОтчета Цикл
		...
		НомерЯчейкиБРО = НомерПоказателяБРО(Показатель, ИмяРедакции);
		РазделБРО = Контейнер[ИмяРаздела];
		ПоместитьЗначениеПоказателяНаСтраницуБРО(РазделБРО[0], НомерЯчейкиБРО, Неопределено);
...
// При этом в сокрашенный протокол включаются
...
Функция ЭтоКонсолидируемыйРазделБРО(ОписаниеПоказателя, ИмяРедакции)
	...
	// Это устаревшая технология, которая в следующих редакциях формы не планируется к использованию
...
//  ИмяРедакции      - Строка - Имя редакции отчета в терминах БРО
//
Процедура ВключитьСтраницу(СтраницаБРО, ОписаниеСтраницы, ИмяРедакции)
	...
	НомерСлужебнойЯчейки = НомерСлужебнойЯчейкиПризнакНалогоплательщика(
		ОписаниеСтраницы.НомерЛиста,
		ОписаниеСтраницы.НомерПриложения,
		ИмяРедакции);
