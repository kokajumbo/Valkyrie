	...
	Если Операция = "getPrivateLNData" Тогда
		...
		Если КодОтвета = "1" Или Не ЗначениеЗаполнено(Результат.ТекстСообщенияФСС) Тогда
			Если ТипЗнч(Документ) = Тип("Структура") Тогда
				// Метод вызван только для получения Хэша ЭЛН.
...
Процедура ЗагрузитьРезультатПолученияЭЛН(Документ, LN_ROW, Отказ)
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	ЗагрузитьХэшЭЛН(Документ, LN_ROW, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьХэшЭЛН(Документ, LN_ROW, Отказ)
	Если Не ЗначениеЗаполнено(Документ.Организация) Тогда
		ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Метаданные.Документы.БольничныйЛист, "Организация");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Документ.ФизическоеЛицо) Тогда
		ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Метаданные.Документы.БольничныйЛист, "ФизическоеЛицо");
	КонецЕсли;
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
