...
Процедура ЗаполнениеОсновнойЧастиУКД(ДанныеДляФормированияЭД, СтруктураЭД, ДеревоДанных)
	...
	Для Каждого Строка Из ДанныеДляФормированияЭД.ТаблицаДокумента Цикл
		...
		Если ТолькоУслуги
			И ПризнакТовараЕдиногоДокумента(НоваяСтрока.Товар) = "1" Тогда
			ТолькоУслуги = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьОсновнуюЧастьУПД_2019(ДанныеДляФормированияЭД, СтруктураЭД, ДеревоДанных)
	...
	Для каждого Строка Из ДанныеДляФормированияЭД.ТаблицаДокумента Цикл
		...
		ЭлектронноеВзаимодействиеМОТП.ЗаполнитьСведенияОМаркировке(НоваяСтрока, Строка, ТаблицаКодовМаркировки);
...
Функция ПодготовитьСтруктуруДляПоступленияТоваровУслугУПД_2019(ДеревоДанных)
	...
	ШтрихкодыУпаковок = ЭлектронноеВзаимодействиеМОТП.НоваяТаблицаШтрихкодыУпаковок();
	...
	ЭлектронноеВзаимодействиеМОТП.СвернутьТаблицуШтрихкодовУпаковок(ШтрихкодыУпаковок);
...
Функция СтруктураДляСопоставленияНоменклатурыЭД(Номенклатура)
	...
	ЗначенияРеквизитовНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура,
		"Наименование, Артикул, ВидСтавкиНДС, ЕдиницаИзмерения, ЕдиницаИзмерения.Наименование, ЕдиницаИзмерения.Код");
