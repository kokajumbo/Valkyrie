...
Процедура ПриОткрытии(Параметры) Экспорт
	...
	Если ТипЗнч(Параметры) = Тип("УправляемаяФорма") Тогда
...
Процедура ДекорацияСостояниеЭДОНажатие(Форма, СтандартнаяОбработка = Ложь) Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Иначе
			...
			Иначе
				ТекстВопроса = НСтр("ru='Для того чтобы настройть ЭДО с контрагентом, нужно записать документ. Записать?'");
...
Процедура ПредложениеОформитьЗаявлениеНаПодключение(ПараметрыПодключения = Неопределено,
	...
	ОткрытьФорму("ОбщаяФорма.ПредложениеОформитьЗаявлениеНаПодключение", ПараметрыОткрытия, ВладелецФормы,,,,
