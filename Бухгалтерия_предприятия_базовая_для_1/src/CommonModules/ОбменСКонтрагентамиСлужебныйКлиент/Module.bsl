...
Процедура ПригласитьКОбменуЭДО(ПараметрКоманды, ПараметрыВыполненияКоманды = Неопределено) Экспорт
	...
	Если МассивСсылок.Количество() = 1 Тогда
		ПараметрыОткрытия.Вставить("Контрагент", МассивСсылок[0]);
	...
	Иначе
		...
		ОткрытьФорму("РегистрСведений.ПриглашенияКОбменуЭлектроннымиДокументами.Форма.ПомощникМассовойОтправкиПриглашений",
			ПараметрыОткрытия);
	КонецЕсли;
...
Процедура НачатьВыполнениеОперацииЭДО(Знач ОперацияЭДО, Знач ОбработкаЗавершения) Экспорт
	
	ОбменСКонтрагентамиСлужебныйВызовСервера.ОбновитьКешиОператоровЭДОИФорматов();
	
	ПроцессВыполнения = НовыйПроцессВыполненияОперацииЭДО(ОперацияЭДО, ОбработкаЗавершения);
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДОЗавершение(Контекст)
	...
	ФормаВладелец = Неопределено;
	Если ТипЗнч(Контекст.Оповещение.Модуль) = Тип("УправляемаяФорма") Тогда
...
Процедура ПолучитьФайлШаблонаСоглашенияНаРоуминг(ДлительнаяОперация, Контекст)
	...
	ПолучитьФайл(ДлительнаяОперация.АдресРезультата, НСтр("ru = 'Письмо о согласии на использование роуминга.docx'"));
...
Процедура ВыбратьФайлСоглашенияНаРоумингЧерезДиалогВыбора(ПомещенныеФайлы, Контекст) Экспорт
	...
	Контекст.ИмяФайлаСоглашенияНаРоуминг = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(
		ПомещенныеФайлы[0].Имя).Имя;
...
Функция НовыйПроцессВыполненияОперацииЭДО(Знач ОперацияЭДО, Знач ОбработкаЗавершения)
...
Процедура ВыполнитьОперациюЭДО(Знач РезультатТекущегоЭтапа, Знач ПроцессВыполнения) Экспорт
	...
	Если РезультатВыполнения = Неопределено Тогда
		...
		ЭтотМетод = Новый ОписаниеОповещения("ВыполнитьОперациюЭДО", ЭтотОбъект, ПроцессВыполнения);
		ЗапуститьВыполнениеОперацииЭДО(ОперацияЭДО, ЭтотМетод);
		Возврат;
	КонецЕсли;
...
Процедура ЗапуститьВыполнениеОперацииЭДО(Знач ОперацияЭДО, Знач ОбработкаПродолжения)
	...
	Если ОперацияЭДО.Действие = ОперацияПодключенияЭДО.Действие Тогда
		...
		НачатьПодключениеЭДО(ОперацияЭДО, ОбработкаПродолжения);
...
Процедура НачатьПодключениеЭДО(Знач ОперацияЭДО, Знач ОбработкаПродолжения)
	...
	Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
		...
		НачатьПодключениеЭДО_1СЭДО(ОперацияЭДО, ОбработкаПродолжения);
		
	ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") Тогда
		
		НачатьПодключениеЭДО_Такском(ОперацияЭДО, ОбработкаПродолжения);
...
Процедура НачатьПодписаниеРегистрационногоПакета1СЭДО(Знач ДанныеПакета, Знач ОбработкаПродолжения)
	...
	ОткрытьФорму("РегистрСведений.УчетныеЗаписиЭДО.Форма.ПодписаниеРегистрационногоПакета1СЭДО",
...
Процедура НачатьПодключениеЭДО_1СЭДО(Знач ОперацияЭДО, Знач ОбработкаПродолжения)
	
	ПроцессВыполнения = НовыйПроцессВыполненияПодключениеЭДО_1СЭДО(ОперацияЭДО, ОбработкаПродолжения);
	
	ВыполнитьПодключениеЭДО_1СЭДО(Неопределено, ПроцессВыполнения);
	
КонецПроцедуры

Функция НовыйПроцессВыполненияПодключениеЭДО_1СЭДО(Знач ОперацияЭДО, Знач ОбработкаЗавершения)
...
Функция ВыполнитьПодключениеЭДО_1СЭДО_ОтправкаЗаявкиНаПодключение(ПроцессВыполнения)
	...
	Если ОтправкаЗаявки = Неопределено И Не ЗначениеЗаполнено(НомерЗаявки) Тогда
		...
		ПараметрыЗаявки.Организация = ОперацияЭДО.Параметры.Организация;
		ПараметрыЗаявки.ОператорЭДО = ОперацияЭДО.Параметры.ОператорЭДО;
		ПараметрыЗаявки.АдресОрганизации = ОперацияЭДО.Параметры.АдресОрганизации;
		ПараметрыЗаявки.СертификатКриптографии = ОперацияЭДО.Параметры.Сертификат;
		ПараметрыЗаявки.ЭлектроннаяПочта = ОперацияЭДО.Параметры.ЭлектроннаяПочтаДляУведомлений;
		ПараметрыЗаявки.КодНалоговогоОргана = ОперацияЭДО.Параметры.КодНалоговогоОргана;
		ПараметрыЗаявки.СертификатыПодписейОрганизации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОперацияЭДО.Параметры.Сертификат);
...
Функция ВыполнитьПодключениеЭДО_1СЭДО_ОжиданиеОбработкиЗаявкиНаПодключение(ПроцессВыполнения)
	...
	Если ОжиданиеПодключения = Неопределено И Не ЗначениеЗаполнено(ИдентификаторЭДО) Тогда
		...
		ОбработкаПродолжения = Новый ОписаниеОповещения("ВыполнитьПодключениеЭДО_1СЭДО", ЭтотОбъект, ПроцессВыполнения);
		НомерЗаявки = ОперацияЭДО.Результат.НомерЗаявки;
		НачатьОжиданиеОбработкиЗаявкиНаПодключениеЭДО_1СЭДО(НомерЗаявки, ОбработкаПродолжения);
		Возврат Ложь;
...
Процедура НачатьОжиданиеОбработкиЗаявкиНаПодключениеЭДО_1СЭДО(Знач НомерЗаявки, Знач ОбработкаПродолжения, Знач ПараметрыОжидания = Неопределено)
...
Процедура ЗакончитьОжиданиеОбработкиЗаявкиНаПодключениеЭДО_1СЭДО(Знач Результат, Знач Контекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Статус = "Выполнено" Тогда
			ИдентификаторЭДО = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	...
	Если ЗначениеЗаполнено(ИдентификаторЭДО) Тогда
		РезультатОжидания.Выполнено = Истина;
...
Процедура НачатьПодключениеЭДО_Такском(Знач ОперацияЭДО, Знач ОбработкаПродолжения)
	
	ПроцессВыполнения = НовыйПроцессВыполненияПодключениеЭДО_Такском(ОперацияЭДО, ОбработкаПродолжения);
	
	ВыполнитьПодключениеЭДО_Такском(Неопределено, ПроцессВыполнения);
	
КонецПроцедуры

Функция НовыйПроцессВыполненияПодключениеЭДО_Такском(Знач ОперацияЭДО, Знач ОбработкаЗавершения)
...
Процедура НачатьПолучениеНовогоИдентификатораТакском(Знач Параметры, Знач ОбработкаПродолжения)
	...
	Подключение1СТакскомКлиент.ПолучитьУникальныйИдентификаторАбонента(
		Параметры.Сертификат, Параметры.Организация, ОбработкаЗавершения);
