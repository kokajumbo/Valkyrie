	...
	Если СтандартнаяОбработкаПодсистемой <> Ложь Тогда
		...
		Если (ДанныеСобытия.Anchor <> Неопределено) Тогда
			...
			Иначе
				...
				ТолькоСсылка = "";
				Попытка
					ТолькоСсылка = ДанныеСобытия.Anchor.hash; // Не все браузеры поддерживают это свойство.
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='ДанныеСобытия.Anchor.hash = %1'"),
							ТолькоСсылка));
				Исключение
					ТолькоСсылка = "";
					...
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Ошибка получения ДанныеСобытия.Anchor.hash по причине:
								|%1'"),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
				КонецПопытки;
			КонецЕсли;
