...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если Обработки.ОплатаСервиса.ДоступнаОплатаСервиса() И РольДоступна("ПолныеПрава") Тогда
		...
		ФоновоеЗаданиеПроверкаОкончанияПодписки = ДлительныеОперации.ВыполнитьВФоне("Обработки.ОплатаСервиса.ПроверитьОкончаниеПодпискиВФоне",
...
Процедура УстановитьДоступностьОплатыСервиса() Экспорт
	...
	Обработки.ОплатаСервиса.НастроитьИнтерфейсОплатыСервиса(ОплатаДоступна);
...
Процедура УстановитьПараметрыНачалаРаботы(Параметры)
	...
	Если НЕ Константы.ИспользоватьРазделениеПоОбластямДанных.Получить() Тогда
		...
		Если НЕ РезультатПроверки.ПроверкаПройдена Тогда
			Если РезультатПроверки.Ошибка = "НетОрганизации" Тогда
				Параметры.Вставить("АктивироватьПрограмму", Истина);
			ИначеЕсли РезультатПроверки.Ошибка = "НетРегистрационныхДанных" Тогда
				ОшибкаАктивации = НачалоРаботы.АктивироватьАвтоматическиЧерезИнтернет();
				Если ОшибкаАктивации <> Неопределено Тогда
					Параметры.Вставить("ОшибкаПроверкиАктивации", ОшибкаАктивации);
				КонецЕсли;
			Иначе
				Параметры.Вставить("ОшибкаПроверкиАктивации", РезультатПроверки.Ошибка);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
//
// Параметры:
//  Форма        - УправляемаяФорма - содержит реквизит "Список" типа динамический список.
//
