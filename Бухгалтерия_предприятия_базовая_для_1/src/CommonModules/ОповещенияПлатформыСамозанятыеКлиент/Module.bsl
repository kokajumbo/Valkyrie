...
Процедура Подключить() Экспорт
	...
	Если ДоступноФормированиеКомандПодменю() Тогда
...
Процедура ОбработчикФормированияКоманд(ПараметрыКоманд, Команды, КомандаПоУмолчанию, ПараметрыЧата) Экспорт
	...
	Если Не ДоступноФормированиеКомандПодменю() Тогда
		Возврат;
	КонецЕсли;
...
Процедура НачатьПолучениеОповещенийЗавершение(ДлительнаяОперация, ПараметрыЧата) Экспорт
	...
	ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
		...
		ОповещенияПлатформыСамозанятыеВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(ДлительнаяОперация.КраткоеПредставлениеОшибки);
...
Процедура ПолучитьРезультатЗапросаОповещенийЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДлительнаяОперация)
		...
		Иначе
			ОповещенияПлатформыСамозанятыеВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(РезультатЗапроса.Сообщение);
		КонецЕсли;
...
Функция ДоступноФормированиеКомандПодменю()
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ТекущаяВерсияПлатформы = СистемнаяИнформация.ВерсияПриложения;
	МинимальнаяВерсияПлатформы = "8.3.15.0";
	
	Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ТекущаяВерсияПлатформы, МинимальнаяВерсияПлатформы) > 0;
