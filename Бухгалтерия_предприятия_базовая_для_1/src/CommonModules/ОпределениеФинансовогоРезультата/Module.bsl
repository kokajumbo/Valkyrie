...
Функция ПрибылиУбыткиЗакрываемогоГода(Период, Организация)
	...
	Запрос.УстановитьПараметр("Счета",          БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПрибылиИУбытки));
	...
	|	-СУММА(ЕСТЬNULL(ХозрасчетныйОстатки.СуммаОстаток, 0)) > 0 КАК Прибыль,
	|	-СУММА(ЕСТЬNULL(ХозрасчетныйОстатки.СуммаОстаток, 0)) < 0 КАК Убыток
	|ИЗ
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Если ВыборкаИтоги.Прибыль И ВыборкаИтоги.Убыток Тогда
			// По отдельным субсчетам прибыль, по другим - убыток.
			// В этом случае на счете 84 следует отразить сальдированный финансовый результат по всем видам деятельности -
