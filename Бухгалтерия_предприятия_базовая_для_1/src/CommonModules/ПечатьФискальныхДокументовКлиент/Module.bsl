...
Процедура ПроверкаГотовностиККТЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Перем ВыходныеПараметры, ОписаниеОшибки;
	...
	Если Результат.Результат Тогда
		Если Результат.Свойство("ВыходныеПараметры", ВыходныеПараметры) 
			И ВыходныеПараметры <> Неопределено 
			И ВыходныеПараметры.Количество() >= 3
			И ВыходныеПараметры[2] <> 2  Тогда
			
			// Значение ВыходныеПараметры[2] - содержит статус открытия смены для ККТ. 2 - смена открыта.
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ДополнительныеПараметры.ИдентификаторУстройстваЭТ) Тогда
			...
			Если ВыходныеПараметры <> Неопределено Тогда
				ПараметрыОперации.НомерЧека = Формат(ВыходныеПараметры[1]+1, "ЧГ=0");
			Иначе
