...
Процедура ЗаполнитьСписокВыбораПорядкаОтраженияАвансаВТЧ(Форма, Знач ЕстьТабличнаяЧасть)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам
...
Процедура УстановитьУсловноеОформлениеШапкаИВидимость(Форма, НачалоПутиКДаннымТЧ, ФормаДокументаОднострочная)
	...
	// РасшифровкаПлатежаПлатежныеКартыПолеОтражениеДохода
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаПлатежныеКартыПолеОтражениеДохода");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ЭлементУО.Отбор,
