...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(Форма)
	...
	КонецЕсли;
	СписокПредставленийДоходы = Неопределено;
	Если Форма.ПолеОтражениеДоходаВидимость
		И НЕ (ЭтоОднострочнаяФорма И КоличествоСтрок > 1) Тогда
		СписокПредставленийДоходы = Элементы[НазваниеТабличнойЧасти + "ПолеОтражениеДохода"].СписокВыбора;
	КонецЕсли;
	...
	Для каждого СтрокаПлатеж Из РасшифровкаПлатежа Цикл
		...
		Если ЗначениеЗаполнено(СтрокаПлатеж.ПорядокОтраженияАванса) 
			И СписокПредставлений <> Неопределено Тогда
...
Процедура ПатентОбработкаВыбораНаСервере(Форма, ВыбранныйПатент, ДополнительныеПараметры) Экспорт
...
Процедура УправлениеЭлементамиОтраженияВУСННаСервере(Форма)
	...
	Если Не ПриходныйКассовыйОрдерФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма) Тогда
		...
		Элементы.ПредупреждениеОЗаполненииРеквизитовПатента.Видимость = Ложь;
...
Процедура УстановитьУсловноеОформлениеШапкаИВидимость(Форма, ФормаДокументаОднострочная)
	...
	// РасшифровкаВыручкиПолеОтражениеДохода
	
	ЭлементУО = Форма.УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаВыручкиПолеОтражениеДохода");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ЭлементУО.Отбор,
