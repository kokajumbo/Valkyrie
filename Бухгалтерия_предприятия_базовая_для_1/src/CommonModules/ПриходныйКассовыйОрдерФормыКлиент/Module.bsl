	...
	Если НоваяСтрока И НЕ Копирование Тогда
		...
		Если Форма.ПрименениеУСН ИЛИ Форма.ПрименяетсяУСНПатент Тогда
			СтрокаПлатеж.ПолеОтражениеАванса = УчетУСНКлиентСервер.ПредставлениеПорядкаОтраженияАванса(
				...
				Элементы.РасшифровкаПлатежаПолеОтражениеАванса.СписокВыбора);
		КонецЕсли;
		Если Объект.БезЗакрывающихДокументов Тогда
			...
			СтрокаПлатеж.ПолеОтражениеДохода = УчетУСНКлиентСервер.ПредставлениеПорядкаОтраженияАванса(
				СтрокаПлатеж.ПорядокОтраженияАванса,
				Форма.ПорядокОтраженияАвансаСоответствиеЗначений, 
				Элементы[НазваниеТабличнойЧасти + "ПолеОтражениеДохода"].СписокВыбора);
		КонецЕсли;
	КонецЕсли;
...
Процедура РасшифровкаПлатежаПолеОтражениеДоходаПриИзменении(Форма, Элемент) Экспорт
	
	СписокВыбора = Элемент.СписокВыбора;
	СтрокаПлатеж = ПриходныйКассовыйОрдерФормыКлиентСервер.РасшифровкаПлатежаТекущиеДанные(Форма);
	
	Если НЕ ЗначениеЗаполнено(СтрокаПлатеж.ПолеОтражениеДохода) Тогда
		СтрокаПлатеж.ПолеОтражениеДохода = СписокВыбора[0].Значение;
	КонецЕсли;
	
	ВыбранныйЭлемент = СписокВыбора.НайтиПоЗначению(СтрокаПлатеж.ПолеОтражениеДохода);
	Если ВыбранныйЭлемент <> Неопределено Тогда
...
Процедура РасшифровкаПлатежаПолеОтражениеДоходаНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
...
Процедура РасшифровкаПлатежаПолеОтражениеДоходаОбработкаВыбора(Форма, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
