...
Функция ПредопределенныеРазрезыРаспределения()
	...
	РазрезыРаспределения.Вставить("Продукция",            Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
...
Функция СоздатьНовыеРасходы(МенеджерВременныхТаблиц, ПравилоРаспределения, МаксимальныйИдентификатор, РазрезыРаспределения)
	...
	// Затем из таблицы отбираются _новые_ наборы аналитики расходов (для исключения старых добавляется соединение по полям разрезов распределения)
	...
	ОператорЗапроса = ОписаниеЗапросаСозданиеТаблицы.Операторы[0];
