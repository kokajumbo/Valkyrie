...
Процедура НалоговыйПериодОбработкаВыбора(Форма, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
	...
	Если ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоТорговыйСбор(Форма.ВидНалога)
			ИЛИ ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоНалогУСН(Форма.ВидНалога) Тогда
		ВидПериода = "Квартал";
	КонецЕсли;
	Периодичность = ПредопределенноеЗначение("Перечисление.Периодичность." + ВидПериода);
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		...
		Если Объект.БезЗакрывающихДокументов Тогда
			СтрокаПлатеж.ПолеОтражениеДохода = УчетУСНКлиентСервер.ПредставлениеПорядкаОтраженияАванса(
				...
				Элементы.РасшифровкаПлатежаПолеОтражениеДохода.СписокВыбора);
		КонецЕсли;
	КонецЕсли;
...
Процедура РасшифровкаПлатежаПолеОтражениеДоходаПриИзменении(Форма, Элемент) Экспорт
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		СтрокаПлатеж.ПорядокОтраженияДохода = Форма.ПорядокОтраженияДоходаСоответствиеЗначений[СписокВыбора.Индекс(ВыбранныйЭлемент)];
		РасходныйКассовыйОрдерФормыКлиентСервер.ЗаполнитьРеквизитыРасшифровкаПлатежа(Форма, Истина, СтрокаПлатеж);
	КонецЕсли;
