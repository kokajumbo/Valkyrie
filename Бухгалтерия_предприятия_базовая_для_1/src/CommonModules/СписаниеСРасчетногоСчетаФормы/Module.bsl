...
Процедура ПодготовитьФормуНаСервере(Форма) Экспорт
	...
	НазваниеТабличнойЧасти = СписаниеСРасчетногоСчетаФормыКлиентСервер.НазваниеТаблицыРасшифровкиПлатежа(Форма);
	БанкИКассаФормы.ПодготовитьРасчетыПриОплате(Форма, НазваниеТабличнойЧасти);
...
Процедура ДатаПриИзмененииСервер(Форма) Экспорт
	...
	НазваниеТабличнойЧасти = СписаниеСРасчетногоСчетаФормыКлиентСервер.НазваниеТаблицыРасшифровкиПлатежа(Форма);
	БанкИКассаФормы.УстановитьРасчетыПриОплате(Форма, НазваниеТабличнойЧасти);
	
	ЗаполнитьВидНалоговогоОбязательстваНаСервере(Форма);
	
	ЗаполнитьПорядокОтраженияУплатыНалогаНаСервере(Форма);
	
	ЗаполнитьДобавленнуюКолонкуПолеОтражениеДохода(Форма);
...
Процедура УстановитьВидимостьСтраниц(Форма) Экспорт
	...
	Иначе
		...
		РасшифровкаНедоступна = Объект.БезЗакрывающихДокументов 
			И НЕ Форма.ПолеОтражениеДоходаВидимость 
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(Форма) Экспорт
	...
	ЗаполнитьДобавленнуюКолонкуПолеОтражениеДохода(Форма, РасшифровкаПлатежа);
	
КонецПроцедуры

Процедура ЗаполнитьДобавленнуюКолонкуПолеОтражениеДохода(Форма, РасшифровкаПлатежа = Неопределено) Экспорт
	
	Если НЕ Форма.ПолеОтражениеДоходаВидимость Тогда
		Возврат;
	КонецЕсли;
	...
	СписокПредставлений = Форма.Элементы.РасшифровкаПлатежаПолеОтражениеДохода.СписокВыбора;
	Для каждого СтрокаПлатеж Из РасшифровкаПлатежа Цикл
		Если ЗначениеЗаполнено(СтрокаПлатеж.ПорядокОтраженияДохода) Тогда
			СтрокаПлатеж.ПолеОтражениеДохода = УчетУСНКлиентСервер.ПредставлениеПорядкаОтраженияАванса(
				СтрокаПлатеж.ПорядокОтраженияДохода, Форма.ПорядокОтраженияДоходаСоответствиеЗначений, СписокПредставлений);
		КонецЕсли;
	КонецЦикла;
...
Функция СвойстваСтрокРасшифровкиПлатежа(Знач ПараметрыОбъекта, Знач ПолучатьДоговор) Экспорт
	...
	Если ПараметрыОбъекта.БезЗакрывающихДокументов Тогда
		СвойстваПлатежа.ПорядокОтраженияДохода = НалоговыйУчетУСН.ПорядокОтраженияАванса(
...
Процедура НалогПриИзмененииНаСервере(Форма, ПричиныИзменения, СтрокаПоискаПоКБК) Экспорт
	...
	ИначеЕсли ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоТорговыйСбор(Форма.ВидНалога) Тогда
		Объект.НалоговыйПериод = НачалоКвартала(ДобавитьМесяц(Объект.Дата, -1));
	Иначе
