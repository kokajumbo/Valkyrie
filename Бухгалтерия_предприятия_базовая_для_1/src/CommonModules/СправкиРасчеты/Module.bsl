...
Функция КонтекстФормированияОтчета(Форма)
	...
	НаборыСуммовыхПоказателейДляСовместимости = ОтчетИспользуетНаборыСуммовыхПоказателейДляСовместимости(ИдентификаторОтчета);
	...
	Если НаборыСуммовыхПоказателейДляСовместимости Тогда
...
Функция РазрешенныеНаборыСуммовыхПоказателей(ПоддерживаемыеНаборы)
	...
	Если Константы.НеИспользуетсяОСНО.Получить() Тогда
		...
		НомерНабораСуммовыхПоказателей = СправкиРасчетыКлиентСервер.НомерНабораСуммовыхПоказателейПоУмолчанию();
		Если ПоддерживаемыеНаборы.Найти(НомерНабораСуммовыхПоказателей) = Неопределено Тогда
		...
		Иначе
			Возврат ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НомерНабораСуммовыхПоказателей);
		КонецЕсли;
