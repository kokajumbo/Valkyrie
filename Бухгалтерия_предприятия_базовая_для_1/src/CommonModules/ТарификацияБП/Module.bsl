...
Процедура ДополнитьОписаниеТарифицируемыхУслуг1СОтчетность(ПоставщикиУслуг) Экспорт
	...
	НовыйПоставщик = Новый Структура;
	НовыйПоставщик.Вставить("Идентификатор", ИнтернетПоддержкаПользователейКлиентСервер.ИдентификаторПоставщикаУслугПортал1СИТС());
	НовыйПоставщик.Вставить("Наименование",  НСтр("ru = 'Портал 1С:ИТС'"));
	НовыйПоставщик.Вставить("Услуги",        Новый Массив);
	
	Услуга = Новый Структура;
	Услуга.Вставить("Идентификатор", ИдентификаторУслуги1СОтчетность());
	Услуга.Вставить("Наименование",  НСтр("ru = 'Лицензия для сдачи отчетности одного юридического лица'"));
	Услуга.Вставить("ТипУслуги",     Перечисления.ТипыУслуг.Безлимитная);
...
Процедура РазместитьИнформациюОбОграниченииПоКоличествуОбъектов(Форма) Экспорт
	...
	ИначеЕсли НЕ ТарификацияБПВызовСервераПовтИсп.РазрешенЭлектронныйДокументообротСКонтролирующимиОрганами() Тогда
...
Функция ИдентификаторАктуальногоТарифа() Экспорт
	...
	Если Обработки.ОплатаСервиса.ЕстьВыставленныйСчетНаОплатуСервиса() Тогда
		УстановитьПривилегированныйРежим(Истина);
...
Функция ЕстьВыставленныйСчетНаОплатуСервиса() Экспорт
	...
	Возврат Обработки.ОплатаСервиса.ЕстьВыставленныйСчетНаОплатуСервиса();
...
Функция ФункциональностьОграничиваетсяТарифом() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		Или ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
