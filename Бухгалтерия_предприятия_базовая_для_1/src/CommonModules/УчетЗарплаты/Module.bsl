	...
	Если УчетнаяПолитика.ПрименяетсяУСН(Организация, ПериодРегистрации) Тогда
		...
		Для Каждого СтрокаТаблицы ИЗ НачисленныйНДФЛ Цикл
			...
			Иначе
				...
				Если НачисленныйНДФЛСуммаНалогаИсходный <> 0 Тогда
					МассивНовыхСумм = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(НачисленныйНДФЛСуммаНалогаИсходный, МассивСумм);
					Если МассивНовыхСумм <> Неопределено Тогда
						НомерСтроки = 0;
						Для Каждого СтрокаМассива ИЗ МассивОтбора Цикл
							СтрокаМассива.Сумма = СтрокаМассива.Сумма - МассивНовыхСумм[НомерСтроки];
...
Процедура СинхронизироватьПодразделенияИРегистрацииВНалоговомОргане(ТаблицаЗаработнойПлаты, Реквизиты)
	...
	Для Каждого СтрокаТаблицы ИЗ ПромежуточнаяТаблица Цикл
		...
		Для Каждого СтрокаМассива ИЗ МассивУдержаний Цикл
			МассивНовыхСумм = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(СтрокаМассива.Сумма, МассивКоэффициентов);
			Если МассивНовыхСумм <> Неопределено Тогда
...
Процедура УстановитьНастройкиУчетаЗарплаты(ВестиУчетЗарплатыИКадровВоВнешнейПрограмме) Экспорт
	...
	Если ВестиУчетЗарплатыИКадровВоВнешнейПрограмме Тогда
		Константы.ИспользоватьКадровыйУчет.Установить(Ложь);
	Иначе
		Константы.РасчетЗарплатыДляНебольшихОрганизаций.Установить(РасчетЗарплатыДляНебольшихОрганизаций.РасчетЗарплатыДляНебольшихОрганизацийВозможен()); 
...
Функция МинимальнаяОплатаТрудаРФ(Период) Экспорт
	
	Попытка
		
		ДанныеМРОТ = РегистрыСведений.МинимальнаяОплатаТрудаРФ.ДанныеМинимальногоРазмераОплатыТрудаРФ(Период);
		Возврат ДанныеМРОТ.Размер;
		
	Исключение
		
		// Если регистр сведений "МинимальнаяОплатаТрудаРФ" еще не заполнен -
		// попытаемся получить размер МРОТ из его поставляемых данных.
		// Если в поставляемых данных сведения также отсутствуют - будет вызвано исключение.
		
		Возврат МинимальнаяОплатаТрудаРФИзПоставляемыхДанных(Период);
		
	КонецПопытки;
	
КонецФункции

Функция МинимальнаяОплатаТрудаРФИзПоставляемыхДанных(Период)
	
	ПоставляемыйМакет = РегистрыСведений.МинимальнаяОплатаТрудаРФ.ПолучитьМакет("ПредопределенныеЗначения");
	ПоставляемыеСведения = ОбщегоНазначения.ЗначениеИзСтрокиXML(ПоставляемыйМакет.ПолучитьТекст());
	
	ТаблицаСтавокМРОТ = ПоставляемыеСведения.Данные;
	ТаблицаСтавокМРОТ.Сортировать("Период Убыв");
	
	Для Каждого СтрокаСтавки Из ТаблицаСтавокМРОТ Цикл
		
		Если СтрокаСтавки.Период <= Период Тогда
			Возврат СтрокаСтавки.Размер;
		КонецЕсли;
		
	КонецЦикла;
	
	// Ничего не нашли, сообщаем об ошибке.
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Сведения о минимальном размере оплаты труда РФ за %1 год отсутствуют'"),
