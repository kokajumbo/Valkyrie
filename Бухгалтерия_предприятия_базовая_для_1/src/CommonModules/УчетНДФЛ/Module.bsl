...
//			если задан - движения будут сформированы только по переданнм физическим лицам.
//		РегистрацияВНалоговомОргане - Массив - массив элементов справочника РегистрацииВНалоговомОргане, необязательный, 
//			если задан - движения будут сформированы только по переданнм регистрациям.
//		Записывать - булево - признак того, надо ли записывать движения сразу, или они будут записаны позже.
//
...
//
Процедура СформироватьУдержанныйНалогПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, УдержанныйНалог, РегистрацияВНалоговомОргане = Неопределено, Записывать = Ложь, ФормироватьРасчетыНалоговыхАгентовСБюджетом = Истина) Экспорт
...
//		Записывать - булево - признак того, надо ли записывать движения сразу, или они будут записаны позже.
//
Процедура СформироватьДокументыУчтенныеПриРасчетеДляМежрасчетногоДокументаПоВременнойТаблице(Движения, Отказ, Организация, МенеджерВременныхТаблиц, Регистратор, ИмяТаблицыФизлиц = "ВТФизическиеЛица", Записывать = Ложь) Экспорт
...
Функция РассчитатьНалогПоОсновнойСтавке(ИсключаемыйРегистратор, Организация, МесяцРасчета, Знач СписокФизическихЛиц, Знач ПервыйМесяцНалоговогоПериода = Неопределено, Знач ПоследнийМесяцНалоговогоПериода = Неопределено, ОкончательныйРасчет = Истина, ДатаУдержанияИсчисленногоНалога = Неопределено, ДвиженияТекущегоРегистратора = Неопределено) Экспорт	
	...
	// 0. Проверим, стОит ли вообще что-то считать: массив физ.лиц может быть пустым, либо доходы межрасчетного регистратора могут быть отрицательными.
	// В обоих случаях результатом расчетов окажется пустая таблица.
	...
	Если ЕстьДвиженияТекущегоРегистратораПоДоходам Тогда
		...
		Если ДоходыТекущегоРегистратора.Колонки.Найти("Сторно") = Неопределено Тогда
			ДоходыТекущегоРегистратора.Колонки.Добавить("Сторно", Новый ОписаниеТипов("Булево"))
		КонецЕсли;
		Если Не ОкончательныйРасчет Тогда
			ТаблицаДоходовТекущегоРегистратора = ДоходыТекущегоРегистратора.Скопировать(ДоходыТекущегоРегистратора.НайтиСтроки(Новый Структура("Сторно", Ложь)), "ФизическоеЛицо, СуммаДохода, СуммаВычета");
	...
	//	Из регистра расчета СведенияОДоходахНДФЛ получим записи, соответствующие условиям:
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОкончательныйРасчетДоходы", 
	...
	Если ЕстьДвиженияТекущегоРегистратораПоДоходам Тогда
		ДоходыТекущегоРегистратора.Свернуть("ФизическоеЛицо, МесяцНалоговогоПериода, ДатаПолученияДохода, Подразделение, Период, СтавкаНалогообложения, ДоходМежрасчетногоПериода, ИсточникДоходаЗаПределамиРФ, КодВычета, КодДохода, КатегорияДохода, Сторно", "СуммаДохода, СуммаВычета");
	...
	Если ЭтоРасчетПоДоходамТекущегоМесяца Тогда
		...
		Для Каждого СтрокаТаблицыДоходов Из ДанныеУчетаДоходов Цикл
			...
			Если СтрокаТаблицыДоходов.УчитыватьПериодПриРасчете Тогда
				СтрокаПериодовПоФизическомуЛицу.НачалоГодаНалоговогоПериода = Мин(?(НачалоМесяца(СтрокаТаблицыДоходов.Период) = НачалоМесяцаРасчета, СтрокаТаблицыДоходов.НалоговыйПериод, Макс(НачалоГодаРасчета, НачалоГодаПериодаРасчета)), СтрокаПериодовПоФизическомуЛицу.НачалоГодаНалоговогоПериода);
		...
		КонецЦикла;	
	Иначе
		// В случае если расчет выполняется не по доходам текущего месяца, в качестве НачалоГодаНалоговогоПериода будем использовать начала года первого месяца периода, 
		// переданного на вход алгоритма расчета, в качестве ПоследнийМесяцНалоговогоПериода - первый день месяца окончания периода, переданного на вход алгоритма расчета
	...
	Если РассчитыватьНалогНарастающимИтогом Тогда 
		...
		Пока ИндексОбрабатываемойСтроки <= ДанныеУчетаДоходов.Количество() - 1 Цикл
			...
			Пока ИндексОбрабатываемойСтроки <= ДанныеУчетаДоходов.Количество() - 1 И ДанныеУчетаДоходов[ИндексОбрабатываемойСтроки].ФизическоеЛицо = ТекущееФизическоеЛицо Цикл
				...
				ИначеЕсли ТекущаяСтрока.ДатаПолученияДохода <> ПредыдущаяСтрока.ДатаПолученияДохода Тогда
					ПредыдущаяСтрока.ЭтоПоследняяСтрокаЗаДату = Истина;
					// если на предыдущую дату доход нарастающим итогом с начала месяца равен 0, то соответствующая строка не нужна: в месяце есть еще строки 
		...
		// В качестве результата получаем соответствие, в котором по годам лежат соответствия, в которых каждому вычету сопоставлена структура с полями Размер и ОграничениеПоДоходам.
	...
	|		И ДвиженияРегистратора.ГоловнаяОрганизация = &ГоловнаяОрганизация) КАК Авансы
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если РассчитыватьНалогНарастающимИтогом Тогда
		...
		// Однократный обход таблицы ДанныеУчетаДоходов организован двумя вложенными циклами; внутренний цикл выполняет расчет налога за один налоговый период по одному физлицу.
		// В качестве промежуточной структуры данных для результатов расчета за месяц используем две таблицы:
		// в одной оставляем только ресурсы НалогПоСтавке13, ЗачтеноАвансовыхПлатежейПоСтавке13, во второй таблице храним 
		// результат расчета вычетов (измерение КодВычета и ресурс РазмерВычета).
		...
		Пока ИндексТекущейСтрокиТаблицыДоходов <= ДанныеУчетаДоходов.Количество() - 1 Цикл		
			...
			Если ТекущаяСтрокаТаблицыДоходов.ЭтоПерваяСтрокаПоФизическомуЛицу Тогда // Начало обработки очередного физ.лица.
				...
				// В случае с промежуточным расчетом будем использовать промежуточную таблицу, а потом переносить в результирующую с учетом ограничений.
				Если Не ОкончательныйРасчет Тогда
				...
				Если ТекущаяСтрокаТаблицыДоходов.ЭтоПерваяСтрокаЗаМесяц Тогда
					...
					Если ДоходЗаМесяцОстаток > 0 И МесяцНалоговогоПериода <= ТекущаяСтрокаТаблицыДоходов.СтаршийМесяцПолученияДохода Тогда
						...
						Если ДоходЗаМесяцОстаток > 0 Тогда
							// Заполняем таблицу значений ВычетыЗаМесяц (с колонками КодВычета, Сумма) суммами вычетов, которые будут применены в текущем месяце.
							// Строки в таблицу помещаем в соответствии с наиболее выгодным для сотрудника приоритетом применения вычетов (описанным непосредственно в справочнике вычетов), для чего.
							// отсортируем таблицу ТаблицаВычетов по полю ПорядокСписания.
							// Применяемые суммы вычетов переписываем из таблицы Остатки, содержащей права на вычеты, в пределах доходов, которые могут быть уменьшены на стандартные, имущественные вычеты.
							// Если положенных физ.лицу вычетов оказывается больше, чем начисленного дохода, в таблице Остатки оказываются суммы неиспользованных в этом месяце вычетов для предоставления их в будущем.
							...
							Для Каждого ВычетКСписанию Из ОстаткиВычетов.ТаблицаВычетов Цикл
								...
								Если ВычетЗаМесяц <> 0 Тогда
									СтрокаТаблицыРезульта = ВычетыЗаМесяц.Добавить();	
									СтрокаТаблицыРезульта.КодВычета = ВычетКСписанию.КодВычета;
									СтрокаТаблицыРезульта.РазмерВычета = ВычетЗаМесяц;
					...
					// Начнем расчет с вычитания уже зарегистрированных в учете вычетов, затем по мере расчета вычеты будут "применяться" к конкретным доходам.
				...
				Если ПроводитьРасчетЗаОчереднойМесяц Тогда
					...
					Если ТекущаяСтрокаТаблицыДоходов.ЭтоПоследняяСтрокаЗаДату Тогда
						...
						Если ВсегоВычетовЗаДату <> 0 Тогда
							...
							// Таблица ВычетыЗаМесяц содержит строки с располагаемыми суммами вычетов, упорядоченные в соответствии с наиболее выгодным для сотрудника приоритетом применения вычетов.
							// Переписываем вычеты из таблицы ВычетыЗаМесяц в таблицу ВспомогательнаяТаблицаВычетовМесяца (распределяя суммы по подразделениям из доходов этой даты получения дохода) 
							// в пределах рассчитанного ранее примененного за дату вычета (если сумма вычета и остаток вычета на дату одного знака, берем минимальное по модулю).
							// Если располагаемая сумма вычета полностью исчерпана (стала равна 0), удаляем строку из таблицы ВычетыЗаМесяц.
...
//				ФизическоеЛицо: должно быть непустым
//				МесяцНалоговогоПериода, должно быть непустым
//              колонки сумм исчисленного налога (НалогПоСтавке13) и сумм зачтенных авансовых платежей (для работающих по патенту, ЗачтеноАвансовыхПлатежейПоСтавке13), их имена могут быть переданы дополнительно
//				может содержать другие колонки
//		ИмяКолонкиНалогаИсчисленного - строка -
//		ИмяКолонкиАвансаЗачтенного - строка -
//
...
Процедура ДополнитьДанныеУчетаСтавками(ДанныеУчета, ПорядокУчетаСтатусаНаКонецГода = "НеУчитыватьИзмененияСтатуса", ОкончаниеОтчетногоПериода = '00010101', СтатусыФизическихЛиц = Неопределено, Знач ПервыйМесяцНалоговогоПериода = '00010101', Знач ПоследнийМесяцНалоговогоПериода = '00010101')
	...
	// Т.к. ставка налогообложения, необходимость учитывать доход нарастающим итогом и необходимость учитывать вычет к доходу (если он есть), 
	// зависит от небольшого количества параметров (статус налогоплательщика, категория дохода, налоговый период, к которому относится дата дохода и т.д), 
	// различных комбинаций значений, которых на практике при расчете встречается не так уже и много, будем использовать готовую таблицу соответствий.
...
Процедура СоздатьВТВычетыКДоходамФизическихЛиц(Ссылка, Организация, ДатаПолученияДохода, МенеджерВременныхТаблиц, ИмяВТНачисления = "ВТНачисления", ИмяВТПредоставленныеВычеты = "ВТПредоставленныеВычеты") Экспорт
	...
	|						ВТВычетыПоДоходам КАК ГодовойВычетПоДоходам
	|					ГДЕ
	|						ГодовойВычетПоДоходам.ГодовойВычет <> 0)) КАК ВычетыПредоставленные
	|
	|СГРУППИРОВАТЬ ПО
...
//
// Параметры:
//      СтрокиНДФЛЗаГод - Массив, ТаблицаЗначений - массив содержит строки таблицы значений, в обоих случаях обязательны колонки НалогПоСтавке13 и ЗачтеноАвансовыхПлатежейПоСтавке13 
...
// Вписывает переданную сумму вычета в соответствие ОстаткиВычетов.Остатки, добавляя к существующему значению или создавая новое описание вычета.
...
// Вписываемый налог должен иметь тот же знак, что и доход. Если в результате расчета налога нарастающим итогом за год получилось иначе, 
// вписывается нулевая сумма налога (возможно из строк за несколько месяцев) и сумма вычетов в пределам суммы доходов документа.
//
// Параметры:
...
Процедура ОбработатьНаборыЗаписейРегистраНакопленияПоРезультатуЗапроса(ИмяРегистра, Результат, ПараметрыОбновления = Неопределено) 
...
Процедура УточнитьКатегориюДоходаАвансовогоПлатежа(ПараметрыОбновления = Неопределено) Экспорт
	...
	ОбработатьНаборыЗаписейРегистраНакопления("РасчетыНалогоплательщиковСБюджетомПоНДФЛ", Текст, , , ПараметрыОбновления)
...
Процедура ПроставитьОснованияПоДоходамИзОпераций(ПараметрыОбновления = Неопределено) Экспорт
	...
	ОбработатьНаборыЗаписейРегистраНакопления("СведенияОДоходахНДФЛ", Текст, , , ПараметрыОбновления)
...
Процедура ЗаполнитьНДФЛКПеречислению(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	Регистратор,
	|	НомерСтроки";
	ОбработатьНаборыЗаписейРегистраНакопления("НДФЛКПеречислению", ТекстЗапроса, , Запрос.МенеджерВременныхТаблиц, ПараметрыОбновления);
...
Процедура ЗаполнитьНДФЛПеречисленный(ПараметрыОбновления = Неопределено) Экспорт
	...
	|УПОРЯДОЧИТЬ ПО
	|	Регистратор";
	ОбработатьНаборыЗаписейРегистраНакопления("НДФЛПеречисленный", ТекстЗапроса, , Запрос.МенеджерВременныхТаблиц, ПараметрыОбновления);
	...
	Если НомерКорректировки = 99 Тогда
		...
		Если Доходы.Количество() > 0 Тогда
			Для Каждого СтрокаДоходы Из Доходы Цикл
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, ПутьКДаннымСотрудникаДляОшибок + ПутьКДаннымДоходов + "[" + Формат(СтрокаДоходы.НомерСтроки - 1, "ЧН=0; ЧГ=") + "].КодДохода",,Отказ);
				Прервать;
			КонецЦикла;
		КонецЕсли;
		...
		Если Вычеты.Количество() > 0 Тогда
			Для Каждого СтрокаВычеты Из Вычеты Цикл
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, ПутьКДаннымСотрудникаДляОшибок + ПутьКДаннымВычетов + "[" + Формат(СтрокаВычеты.НомерСтроки - 1, "ЧН=0; ЧГ=") + "].КодВычета",,Отказ);
				Прервать;
			КонецЦикла;
		КонецЕсли;
