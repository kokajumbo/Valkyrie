...
Функция ЧекЕГАИСXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц, Операция = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(АдресТорговогоОбъекта) Тогда
		АдресИзЗапроса = Новый Структура("АдресТорговогоОбъекта", "");
		ЗаполнитьЗначенияСвойств(АдресИзЗапроса, Шапка);
		АдресТорговогоОбъекта = АдресИзЗапроса.АдресТорговогоОбъекта;
	КонецЕсли;
	// Не ГосИС
