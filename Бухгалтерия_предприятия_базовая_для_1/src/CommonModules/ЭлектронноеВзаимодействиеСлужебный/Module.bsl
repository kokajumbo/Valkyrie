...
Процедура ВывестиРезультатыПроверкиМаршрута(РезультатыПроверки, НастройкаОбмена, Маршрут, Отказ = Ложь,
	ИсключаемыеОшибки = "", Знач ПредставлениеНастройки = "") Экспорт
	...
	Если РезультатыПроверки.Количество() Тогда
		...
		Для Каждого РезультатПроверки Из РезультатыПроверки Цикл
			Если ИсключаемыеОшибки.Найти(РезультатПроверки.Значение) = Неопределено Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, НастройкаОбмена, ИмяПоляОшибки, "Запись", Отказ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения, Знач НастройкиПрокси, Перенаправления, ПараметрыЖурналирования)
	...
	Попытка
		...
		Если ПараметрыЖурналирования <> Неопределено Тогда
			ПараметрыЖурналирования.ОбщийМодуль.ПриОтправкеДанныхHTTP(URL, Соединение, ЗапросHTTP, ПараметрыЖурналирования);
		КонецЕсли;
