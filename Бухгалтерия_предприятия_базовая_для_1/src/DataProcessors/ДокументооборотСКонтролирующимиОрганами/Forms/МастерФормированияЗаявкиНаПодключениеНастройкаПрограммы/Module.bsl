...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Закрыть.Заголовок = НСтр("ru = 'Закрыть'") 
		ИЛИ Элементы.Закрыть.Заголовок = НСтр("ru = 'Завершить подключение к 1С-ЭДО'") Тогда
		ПрограммноеЗакрытие = Истина;
	КонецЕсли;
	
	Если ПодключитьЭДО 
		И НовыйСтатусДокумента = ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Одобрено")
		И Элементы.Закрыть.Заголовок = НСтр("ru = 'Завершить подключение к 1С-ЭДО'") Тогда
...
Процедура УправлениеКнопкамиНавигации()
	...
	Если КнопкаЗакрыть.Заголовок = НСтр("ru = 'Закрыть'") 
		ИЛИ КнопкаЗакрыть.Заголовок = НСтр("ru = 'Завершить подключение к 1С-ЭДО'") Тогда
...
Процедура ЗаполнитьДанныеСлужбыПоддержки()
	...
	// Контактные данные АО "Калуга Астрал"
