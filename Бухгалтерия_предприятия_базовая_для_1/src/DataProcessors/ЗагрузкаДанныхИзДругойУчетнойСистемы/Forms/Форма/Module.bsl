...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ФайловаяСистемаКлиент.ЗагрузитьФайл(ОповещениеЗавершенияВыбораФайла, ПараметрыЗагрузки);
...
Процедура УправлениеФормойПриСозданииНаСервере()
	...
	НадписьПроведениеДокументов = НСтр("ru = 'Проведение документов'");
...
Процедура ОбработатьФайлДляЗагрузки()
	...
	ДлительнаяОперация = ПодготовитьДанныеДляЗагрузки(ПараметрыОперации, УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ПодготовитьДанныеДляЗагрузки(ПараметрыОперации, Знач ИдентификаторФормы)
	
	ДвоичныеДанные = ПолучитьИзВременногоХранилища(ПараметрыОперации.АдресФайла);
	ПараметрыОперации.ХранилищеДанных = Новый ХранилищеЗначения(ДвоичныеДанные);
	УдалитьИзВременногоХранилища(ПараметрыОперации.АдресФайла);
...
Функция НовыеПараметрыОперации()
	...
	// Адрес временного хранилища, в котором хранится прочитанный файл в виде двоичных данных
	Свойства.Вставить("АдресФайла", "");
	// Хранилище значений, в которое на сервере будут помещены данные прочитанного файла из временного хранилища
	Свойства.Вставить("ХранилищеДанных", Неопределено);
	Свойства.Вставить("РасширениеФайла", "");
	Свойства.Вставить("Организация", Неопределено);
	Возврат Свойства;
