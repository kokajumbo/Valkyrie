...
Процедура ОтправитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'При отправке приглашений возникли ошибки.
			|Необходимо выполнить проверку соединения с оператором в учетной записи ЭДО.'");
...
&НаКлиенте
Процедура ОтправитьОтветНаПриглашениеПослеПолученияМаркера(Результат, Контескт) Экспорт 
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		Если Контескт.ОтветНаПриглашение = ОтветНаПриглашение_Принять() Тогда
		...
		ИначеЕсли Контескт.ОтветНаПриглашение = ОтветНаПриглашение_Отклонить() Тогда
		...
		ИначеЕсли Контескт.ОтветНаПриглашение = ОтветНаПриглашение_Отозвать() Тогда
		...
		ПринятьОтклонитьКонтактЧерезОператораЭДОНаСервере(КоличествоОтклоненныхПриглашений,
			Контескт.ВыбранныеИдентификаторы, СоответствиеПрофилейИПараметровСертификатов, ПриглашениеПринято);
	...
	Иначе
		...
		ТекстОшибки = ТекстОшибки + Символы.ПС
			+ НСтр("ru = 'Необходимо проверить соединение с оператором.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьОтветНаПриглашения(ОтветНаПриглашение)
	...
	Если ИдентификаторыОрганизаций.Количество() > 0 Тогда
		...
		Контекст = Новый Структура("ВыбранныеИдентификаторы, ОтветНаПриглашение",
			ВыбранныеИдентификаторы, ОтветНаПриглашение);
		...
		ОбменСКонтрагентамиСлужебныйКлиент.ПолучитьНастройкиЭДОИПараметрыСертификатов(
			ОбработчикОповещения, ИдентификаторыОрганизаций);
