...
Процедура ЗаполнитьУведомлениеОПостановкеНаУчетИП(ПараметрыОтчета, Контейнер) Экспорт
	...
	АдресXML = АдресСтруктурой.ЗначенияПолей;
	РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(АдресСтруктурой.ЗначениеJSON, АдресСтруктурой);
...
Процедура ЗаполнитьУведомлениеОПостановкеНаУчетООО(ПараметрыОтчета, Контейнер) Экспорт
	...
	АдресСтруктурой = УправлениеКонтактнойИнформациейБП.АдресСтруктурой(ПараметрыОтчета.Организация, 
		Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации, , Ложь);
	Контейнер.Лист001.Вставить("АдресXML", АдресСтруктурой.ЗначенияПолей);
	РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(АдресСтруктурой.ЗначениеJSON, АдресСтруктурой);
	ОбработатьЭлементыАдреса(АдресСтруктурой);
	...
	Если УчредителиФизическиеЛица.Количество() > 0 Тогда
		Для каждого СтрокаТаблицы Из УчредителиФизическиеЛица Цикл
			...
			// Адрес
			НовыйЛист.Вставить("АдресXML", СведенияОФизическомЛице.АдресСтруктурой.ЗначенияПолей);
			РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(
				СведенияОФизическомЛице.АдресСтруктурой.ЗначениеJSON, СведенияОФизическомЛице.АдресСтруктурой);
			ОбработатьЭлементыАдреса(СведенияОФизическомЛице.АдресСтруктурой);
	...
	Для каждого СтрокаТаблицы Из ДанныеУчредителей.Учредители Цикл
		...
		Если СтрокаТаблицы.ЭтоЮридическоеЛицо Или КоличествоУчредителей > 1 Тогда
			...
			КонецЕсли;
			// Адрес
			АдресXML = СведенияОЗаявителе.АдресСтруктурой.ЗначенияПолей;
			РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(
				СведенияОЗаявителе.АдресСтруктурой.ЗначениеJSON, СведенияОЗаявителе.АдресСтруктурой);
			ОбработатьЭлементыАдреса(СведенияОЗаявителе.АдресСтруктурой);
...
Процедура ЗаполнитьЗаявлениеРегистрацииИзмененийУчредительныхДокументов(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ПараметрыЗаполнения.Свойство("ДанныеЗаявителя", ДанныеЗаявителя) Тогда
		...
		АдресСтруктурой = УправлениеКонтактнойИнформациейБП.СтруктураАдреса(
			ДанныеЗаявителя.АдресПропискиЗначениеJSON, ДополнительныеПараметрыКИ);
		РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(ДанныеЗаявителя.АдресПропискиЗначениеJSON, АдресСтруктурой);
		ОбработатьЭлементыАдреса(АдресСтруктурой);
		ЗаполнитьЗначенияСвойств(Контейнер.ЛистМ, АдресСтруктурой);
		АдресXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(
			ДанныеЗаявителя.АдресПропискиЗначениеJSON, , Перечисления.ТипыКонтактнойИнформации.Адрес);
...
Процедура ЗаполнитьЗаявлениеРегистрацииИзмененийЕГРЮЛ(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если Изменено.Руководитель Тогда
		...
		ДополнительныеПараметрыКИ = Новый Структура("НаименованиеВключаетСокращение", Ложь);
		АдресСтруктурой = УправлениеКонтактнойИнформациейБП.СтруктураАдреса(
			ДанныеНовогоРуководителя.АдресПропискиЗначениеJSON, ДополнительныеПараметрыКИ);
		РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(
			ДанныеНовогоРуководителя.АдресПропискиЗначениеJSON, АдресСтруктурой);
		ОбработатьЭлементыАдреса(АдресСтруктурой);
		ЗаполнитьЗначенияСвойств(НовыйЛист, АдресСтруктурой);
		АдресXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(
			ДанныеНовогоРуководителя.АдресПропискиЗначениеJSON, , Перечисления.ТипыКонтактнойИнформации.Адрес);
		НовыйЛист.Вставить("АдресXML", АдресXML);
		// Телефон
	...
	Если ПараметрыЗаполнения.Свойство("ДанныеЗаявителя", ДанныеЗаявителя) Тогда
		...
		АдресСтруктурой = УправлениеКонтактнойИнформациейБП.СтруктураАдреса(
			ДанныеЗаявителя.АдресПропискиЗначениеJSON, ДополнительныеПараметрыКИ);
		РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(
			ДанныеЗаявителя.АдресПропискиЗначениеJSON, АдресСтруктурой);
		ОбработатьЭлементыАдреса(АдресСтруктурой);
		ЗаполнитьЗначенияСвойств(НовыйЛист, АдресСтруктурой);
		АдресXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(
			ДанныеЗаявителя.АдресПропискиЗначениеJSON, , Перечисления.ТипыКонтактнойИнформации.Адрес);
