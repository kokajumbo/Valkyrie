...
Процедура УстановитьОтборПоКритериюОтбора()
	...
	// Если не используется RLS, то всегда способ отбора по критерию в подзапросе (вариант 1).
	// Если RLS используется, то для оптимизации открытия формы проверяются варианты 2) и 3).
	// 
	...
	Если Не УправлениеДоступом.ОграничиватьДоступНаУровнеЗаписей()
	 Или Пользователи.ЭтоПолноправныйПользователь() Тогда // не нужна оптимизация отбора по списку документов
	...
	Запрос.УстановитьПараметр("Организации", ОбщегоНазначенияБПВызовСервераПовтИсп.ВсеОрганизацииДанныеКоторыхДоступныПоRLS(Ложь));
	Если КлючНазначенияИспользования = "ДокументыПоДоговоруКонтрагента" Тогда
	...
	Если РезультатАнализа.Пустой() Тогда // достаточно отбора по данным в шапке документа
		...
		ЗапрещенныеПоля = Новый Массив;
		ЗапрещенныеПоля.Добавить("ПараметрыДанных");
		ЗапрещенныеПоля.Добавить(ИмяПоляОтбора);
		Список.УстановитьОграниченияИспользованияВОтборе(ЗапрещенныеПоля);
	...
	Иначе
		...
		Если РазмерПорцииДокументов <> 100 Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "101", Формат(РазмерПорцииДокументов + 1, "ЧГ=0"));
		КонецЕсли;
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
			Список, "ОтборСсылки", СписокДокументов, ОтборИспользование);
...
Процедура ОбработкаОповещенияОЗаписиДокумента(Источник)
	...
	Если СпособОтбора = "ГРАФА" Тогда
		// Переключаемся с отбора по графе на отбор по критерию отбора.
		Если КлючНазначенияИспользования = "ДокументыПоДоговоруКонтрагента" Тогда
			
			ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(
				Список.КомпоновщикНастроек.Настройки.Отбор, "ДоговорКонтрагента");
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
				Список, "ОтборДоговорКонтрагента", ОтборДоговорКонтрагента, ОтборДоговорКонтрагентаИспользование);
		...
		Иначе
			...
			ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(
				Список.КомпоновщикНастроек.Настройки.Отбор, "Информация");
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
				Список, "ОтборКонтрагент", ОтборИнформация, ОтборИнформацияИспользование);
