...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВестиУчетТабачнойПродукцииМОТП Тогда
		ПроверкаИПодборПродукцииМОТП.ПриСозданииНаСервере(ЭтотОбъект);
		ПроверкаИПодборПродукцииМОТПВызовСервера.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект);
	КонецЕсли
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Интеграция МОТП
	Если ВестиУчетТабачнойПродукцииМОТП Тогда
		ПроверкаИПодборПродукцииМОТП.ПослеЗаписиНаСервере(ЭтотОбъект);
	КонецЕсли;
	// Конец Интеграция МОТП
...
Процедура ПрименитьКешПоСтрокеМОТП()
	...
	Если (ТекущаяСтрока = Неопределено) Тогда
		Возврат;
	КонецЕсли;
	ТоварыКешТекущейСтроки = Новый Структура;
	ТоварыКешТекущейСтроки.Вставить("Номенклатура",   ТекущаяСтрока.Номенклатура);
	ТоварыКешТекущейСтроки.Вставить("Характеристика", "");
	ТоварыКешТекущейСтроки.Вставить("Серия",          "");
	
	ТоварыТекущаяСтрока = Новый Структура;
	ТоварыТекущаяСтрока.Вставить("Номенклатура",              ТекущаяСтрока.Номенклатура);
	ТоварыТекущаяСтрока.Вставить("Характеристика",            "");
	ТоварыТекущаяСтрока.Вставить("Серия",                     "");
	ТоварыТекущаяСтрока.Вставить("МаркируемаяПродукцияГосИС", ТекущаяСтрока.МаркируемаяПродукцияГосИС);
	ТоварыТекущаяСтрока.Вставить("СтатусПроверкиГосИС",       ТекущаяСтрока.СтатусПроверкиГосИС);
	...
	ЗаполнитьЗначенияСвойств(ТекущаяСтрока , ТоварыТекущаяСтрока);
