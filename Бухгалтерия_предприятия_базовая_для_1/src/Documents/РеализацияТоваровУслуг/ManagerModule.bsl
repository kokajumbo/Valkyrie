	...
	ИначеЕсли ТипЭД = Перечисления.ТипыЭД.ТоварнаяНакладная Или ТипЭД = Перечисления.ТипыЭД.Прочее Тогда
		...
		Иначе
			...
			Если ДанныеГрузополучателя.Свойства().Получить("СвИП") <> Неопределено
				И ТипЗнч(ДанныеГрузополучателя.СвИП) = Тип("ОбъектXDTO") Тогда // это физ. лицо
...
Функция ТекстЗапросаДанныеДляПечатиЧека(ИменаТаблиц) Экспорт
	...
	ИменаТаблиц.Добавить("ВТ_ОплачиваемаяНоменклатураБезШтрихкода");
	...
	|ПОМЕСТИТЬ ВТ_ОплачиваемаяНоменклатураБезШтрихкода
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Документ КАК Документ,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ДатаДокумента КАК ДатаДокумента,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Номенклатура КАК Номенклатура,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Наименование КАК Наименование,
	|	МАКСИМУМ(ЕСТЬNULL(ШтрихкодыНоменклатуры.Штрихкод, НЕОПРЕДЕЛЕНО)) КАК Штрихкод,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ПлатежныйАгент КАК ПлатежныйАгент,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ДоговорПлатежногоАгента КАК ДоговорПлатежногоАгента,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ЭтоУслуга КАК ЭтоУслуга,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Количество КАК Количество,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.КоличествоОтгружено КАК КоличествоОтгружено,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Цена КАК Цена,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.СтавкаНДС КАК СтавкаНДС,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.СуммаСкидок КАК СуммаСкидок,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Сумма КАК Сумма,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.НомерСтроки КАК НомерСтроки,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.СуммаНДС КАК СуммаНДС,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.КодСтраныПроисхожденияТовара КАК КодСтраныПроисхожденияТовара,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.НомерТаможеннойДекларации КАК НомерТаможеннойДекларации
	|ИЗ
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода КАК ВТ_ОплачиваемаяНоменклатураБезШтрихкода
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|		ПО ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Документ,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ДатаДокумента,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Номенклатура,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Наименование,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ДоговорПлатежногоАгента,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ЭтоУслуга,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Количество,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.КоличествоОтгружено,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Цена,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.СтавкаНДС,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.СуммаСкидок,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.Сумма,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.НомерСтроки,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.СуммаНДС,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.ПлатежныйАгент,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.КодСтраныПроисхожденияТовара,
	|	ВТ_ОплачиваемаяНоменклатураБезШтрихкода.НомерТаможеннойДекларации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
