	...
	Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
		...
		Если ПустаяСтрока(СтрокаТаблицы.Адрес) И ПустаяСтрока(СтрокаТаблицы.Предупреждение) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки, ,
				"Приглашения["+Приглашения.Индекс(СтрокаТаблицы)+"].ЭлектроннаяПочта",, Отказ);
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Предупреждение) Тогда
...
Процедура ИдентификаторОрганизацииПервыйЭтапОткрытие(Элемент, СтандартнаяОбработка)
	...
	КлючЗаписи = КлючУчетнойЗаписиОрганизации(ИдентификаторОрганизации);
...
&НаКлиенте
Функция КлючУчетнойЗаписиОрганизации(ИдентификаторЭДО)
	
	ТипЗначения = Тип("РегистрСведенийКлючЗаписи.УчетныеЗаписиЭДО");
	ПараметрыЗаписи = Новый Массив(1);
	ПараметрыЗаписи[0] = Новый Структура("ИдентификаторЭДО", ИдентификаторЭДО);
...
Процедура ОтправитьПриглашения()
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ПолучитьНастройкиЭДОИПараметрыСертификатов(
		Оповещение, МассивИдентификаторовОрганизации);
	...
	Если ТипЗнч(Результат) = Тип("Структура")
		...
		Для Каждого ИдентификаторЭДО Из МассивИдентификаторовОрганизации Цикл
			...
			Если Соответствие[ИдентификаторЭДО] = Неопределено Тогда
				...
				КлючЗаписи = КлючУчетнойЗаписиОрганизации(ИдентификаторЭДО);
