...
Процедура НастроитьОсновнуюСтраницуФормы(Форма)
	...
	Иначе
		...
		Если Форма.ТребуетсяСоглашение
			...
			Элементы.ПанельСоглашениеНаРоуминг.ТекущаяСтраница = ?(ЗначениеЗаполнено(Форма.ИмяФайлаСоглашенияНаРоуминг),
				Элементы.СтраницаСоглашениеНаРоумингВыбрано, Элементы.СтраницаСоглашениеНаРоумингНеВыбрано);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьПроизвольныйИдентификаторКонтрагентаВСписокВыбора(Форма, Идентификатор, Картинка = Неопределено)
...
Функция ЭлектроннаяПочтаКорректна(Форма)
	...
	Если Форма.ТипИнформацииОКонтрагенте <> ТипИнформацииОКонтрагенте_ЭлектроннаяПочта() Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ОтправитьПриглашение()
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ПолучитьНастройкиЭДОИПараметрыСертификатов(Оповещение,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторОрганизации));
...
Процедура ОтправитьПриглашениеПослеПолученияМаркера(Результат, Контекст) Экспорт
	...
	Если Не (ТипЗнч(Результат) = Тип("Структура")
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Отправка приглашений ЭДО.'"),
			ТекстОшибки,,,КлючЗаписи);
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, КлючЗаписи);
