...
Процедура ПроверкаСоединенияСОператором(Команда)
	...
	Если Модифицированность Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ПроверкаСоединенияСОператоромПродолжить", ЭтотОбъект);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , "Выполнить", НСтр("ru = 'Проверка соединения с оператором'"));
	Иначе
		ПроверкаСоединенияСОператоромПродолжить("Выполнить", Неопределено);
	КонецЕсли;
...
Процедура ВыполнитьСохранениеДанныхВСервисахАутентификация(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		// Пользователь отказался от ввода пароля интернет-поддержки.
		Закрыть = Ложь;
		ДополнительныеПараметры.ИзмененыНастройкиУведомлений = Ложь;
		ЗаписатьИОтправитьВСервисы(ДополнительныеПараметры);
	КонецЕсли;
...
&НаКлиенте
Процедура ПроверкаСоединенияСОператоромПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Состояние(НСтр("ru = 'Проверка соединения с оператором.'"),
		,
		НСтр("ru = 'Выполняется проверка соединения с оператором ЭДО. Подождите...'"));
...
Процедура ПослеПолученияОтпечатковПроверитьСвязьЭДО(Отпечатки, Параметры = Неопределено) Экспорт
	...
	Соответствие = ОбменСКонтрагентамиСлужебныйВызовСервера.СоответствиеДоступныхСертификатовИПараметров(
												МассивОтпечатковСертификатов, Запись.ИдентификаторЭДО);
	...
	Если Не ЗначениеЗаполнено(МассивСертификатов) Тогда
		ТекстСообщения = НСтр("ru = 'Нет доступных сертификатов. Проверка не выполнена.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ПриУспешнойПроверкеПрофиля()
	...
	ТекстСообщения = НСтр("ru = 'Проверка соединения с оператором выполнена успешно.'");
	ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
