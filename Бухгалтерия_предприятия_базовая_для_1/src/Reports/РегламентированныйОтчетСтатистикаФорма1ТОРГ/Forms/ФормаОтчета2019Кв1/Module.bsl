...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Мкт = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2019Кв1_ФормаОтчета");
	...
	Для Стр = 136 По 161 Цикл
		...
		ПараметрыВыгрузки.Вставить("П000701000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
